...
from .bundles import PoeBundle, extract_homepage_version
...
class PoeApi:
    ...
    def load_bundle(self, force_refresh: bool = False):
        logger.debug(
    ...
    def _resolve_bot_id_for_upload(self, bot_candidates: list[str]) -> Optional[int]:
        ...
        for candidate in bot_candidates:
            if not candidate:
                continue
            try:
                bot_info = self.get_botInfo(candidate)
            except Exception as exc:
    ...
    def _emit_upload_filetype_probe(self, file_name: str) -> None:
    ...
    def _emit_upload_action_log(
        ...
        self._emit_upload_filetype_probe(file_name)
    ...
    def send_message(
        ...
        if file_path == []:
            ...
            if not resolved_bot_id or resolved_bot_id <= 0:
                resolved_bot_id = self._resolve_bot_id_for_upload(bot_candidates)
            file_hash_jwts = self.finish_upload(file_form, bot_id=resolved_bot_id)
